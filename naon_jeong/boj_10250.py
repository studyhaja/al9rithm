...
'''
손님 방문 순서 중 호텔 총 층 수인 H의 베수에 해당하는 손님은 가장 위층에 배정된다.
각 호수(라인)의 가장 위층은 (H * 호수)번째 손님에게 돌아간다.
따라서 몇 번째 손님인지를 호텔 총 층 수로 나눈 값이 딱 떨어지면 그 결과값이 호수가 되고 층 수는 H가 된다. 
그 외 경우는 몇 번째 손님인지를 호텔 총 층 수로 나눈 몫에서 1을 더해준 값이 호수고, 나머지는 층이다.
'''
